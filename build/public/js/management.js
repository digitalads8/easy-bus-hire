( function( $ ) {
    ...
    $( function() {
        $( '.es-property-form .js-es-tabs, .es-agency-form .js-es-tabs' ).on(  'tab_changed', function() {
        ...
        $( document ).on( 'click', '.js-es-delete-property, .js-es-delete-agency', function() {
            var $el = $( this );
            var entity_id = $el.data( 'entity-id' );
            
            var action = '';
     
            if ($el.hasClass('js-es-delete-property')) {
                action = 'es_management_delete_property_popup';
            } else if ($el.hasClass('js-es-delete-agency')) {
                action = 'es_management_delete_agency_popup';
            }
            var data = {
                action: action,
        ...
        $('.es-translation-management__select').each(function() {
            var $this = $(this);
            var $trigger = $this.find('.es-translation-management__select-trigger');
            var $options = $this.find('.es-translation-management__options');
            
            $(document).on('click', function(e) {
                if (!$this.is(e.target) && $this.has(e.target).length === 0) {
                    $options.removeClass('selected');
                    $options.parent().removeClass('opened');
                }
            });
    
            $trigger.on('click', function() {
                var isOpened = $this.hasClass('opened');
                $('.es-translation-management-select').removeClass('opened');
                $options.removeClass('selected');
