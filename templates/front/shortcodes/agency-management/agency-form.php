<div class="es-wrap et_smooth_scroll_disabled">
    <div class="es-agency-management es-agency-management--form content-font" id="es-agency-management-form">
        <?php $flashes = es_get_flash_instance( 'prop-management' ); $flashes->render_messages(); ?>
        <form action="" method="POST" class="es-agency-form js-es-ajax-form" enctype="multipart/form-data">
            <?php
            $btn_label = __( 'Submit new agency', 'es' );
            $next_label = _x( 'Next', 'next tab button label', 'es' );
            $back_label = _x( 'Back', 'next tab button label', 'es' );

            if ( ! empty( $id ) ) $btn_label = __( 'Save agency', 'es' );
            ...
            es_framework_view_render( 'tabs', array(
                ...
                'show_logo' => false,
                'before_content_tabs' => "<h3 class='heading-font es-tabs__title'>" . __( 'Add new agency', 'es' ) . "</h3>",
                'after_content_tabs' => $manage_buttons
            ) ); ?>

            <input type="hidden" name="post_id" value="<?php echo $id; ?>"/>
            <?php wp_nonce_field( 'es_save_agency', 'es_save_agency' ); ?>
            <?php wp_nonce_field( 'es_save_agency_ajax', 'es_save_agency_ajax' ); ?>
            <input type="hidden" name="action" value="es_ajax_save_agency"/>
        </form>
    </div>
</div>
