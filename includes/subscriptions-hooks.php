...
function es_subscription_property_check_published( $new_status, $old_status = null, $post = null ) {
	...
	if ( current_user_can( $post->post_author, 'edit_post', $post ) ) {
...
/**
 * @param $data array agency data.
 * @param $agency_id int
 *
 * @return array
 */
function es_subscription_check_agency( $data, $agency_id ) {
	if ( ! ests( 'is_subscriptions_enabled' ) || ! ests( 'is_subscriptions_enabled_agencies' ) ) return $data;
	if ( is_admin()) return $data;
		$agency = es_get_agency( $agency_id );
		$post = $agency->get_wp_entity();
		
		if ( $post->post_type != 'agency' ) return;
		...
		if ( user_can( $post->post_author, 'edit_post', $post ) ) {
			...
			if ( es_user_has_active_subscription( $post->post_author ) ) {
				...
				if ( es_user_can_publish_agencies( $post->post_author ) ) {
					$subscription->increase_agencies();
				} else {
					global $wpdb;
					$wpdb->update( $wpdb->posts, array( 'post_status' => 'draft' ), array( 'ID' => $post->ID ) );
					$flashes->set_message( 'Your subscription plan doesn\'t allow to publish more agencies.', 'es' );
				}
			} else {
				$flashes->set_message( 'Your subscription plan doesn\'t allow to publish agencies.', 'es' );
			}
		}
		return $data;
	}
add_filter( 'es_agency_before_save_data', 'es_subscription_check_agency', 10, 2 );
