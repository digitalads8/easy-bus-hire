                    ...
                    if ( ests( 'price_input_type' ) != 'manual_input' ) {
                        ...
                        foreach ( array( 'min', 'max' ) as $field_range ) {
                            if ( ! empty( $values[ $field_range ] ) ) {
								if ( ! empty( $_COOKIE['currency'] ) && ests( 'is_additional_currencies_enabled' ) ) {
									if ( ! empty( $values[ $field_range ] ) ) {
										foreach ( $values[ $field_range ] as $key => $value ) {
											$values[ $field_range ][ $key ] = number_format( $value, 0, '', ' ');
										}
									}
								} else {
...
function es_enqueue_entity_recaptcha( $entity ) {
	if ( in_array( $entity, array( 'agent', 'agency' ) ) && es_is_entity_contact_btn_enabled( $entity ) ) {
		...
		if ( is_array( $forms ) && in_array( 'request_form', $forms ) ) {
...
function es_order_generate_keywords( $data, $entity ) {
	...
	if ( ! empty( $entity->user_id ) ) {
		...
		if ( $user instanceof WP_User ) {
            foreach ( array( 'user_email', 'user_nicename', 'ID' ) as $key ) {
                if ( ! empty( $user->{$key} ) ) {
	                add_post_meta( $post_id, 'es_order_keywords', $user->{$key} );
                }
            }

			add_post_meta( $post_id, 'es_order_keywords', $post_id );

			if ( $user_entity instanceof Es_Agent_User ) {
				$agent = $user_entity->get_post_entity();
				if ( $agent && $agent->post_title ) {
					add_post_meta( $post_id, 'es_order_keywords', $agent->post_title );
				}
			}

			if ( $user_entity instanceof Es_User && $user_entity->get_full_name() ) {
				add_post_meta( $post_id, 'es_order_keywords', $user_entity->get_full_name() );
			}

		}
	}
}
add_action( 'es_order_after_save_fields', 'es_order_generate_keywords', 10, 2 );
	...
	if ( check_ajax_referer( 'es_save_property_ajax', 'es_save_property_ajax' ) ) {
		...
		if ( ! $is_new || ( es_user_can_publish_listings( get_current_user_id() ) || $is_new ) ) {
			...
			if ( is_wp_error( $saved ) ) {
                ...
                if ( function_exists( 'pll_the_languages' ) && pll_is_translated_post_type( 'properties' ) )  {
                    $property_translation_id = intval( filter_input( INPUT_POST, 'property_lang_original_id' ) );
                
                    if ( isset( $property_translation_id  ) ) {
                        $property_translation_id = sanitize_text_field( $property_translation_id  );
                        $original_post_id = (int) $property_translation_id;
                        $original_language = pll_get_post_language($original_post_id);
                
                        if ( $original_post_id ) {
                            pll_save_post_translations( array( $original_language => $original_post_id, pll_current_language() => $saved ) );
                        }
                    }
                }
...
/**
 * Save agency action from agency management panel.
 *
 * @return void
 */
function es_pm_save_agency_handler() {
	if ( ! wp_doing_ajax() && es_get_nonce( 'es_save_agency' ) ) {
		$valid_request = wp_verify_nonce( es_get_nonce( 'es_save_agency' ), 'es_save_agency' );
		$redirect = esc_url( filter_input( INPUT_POST, '_wp_http_referer' ) );
		...
		if ( $valid_request && ! defined( 'DOING_AJAX' ) ) {
			...
			if ( ! $is_new || ( es_user_can_publish_agencies( get_current_user_id() ) || $is_new ) ) {
				$is_new = empty( $post_id );
				$data = es_clean( $_POST['es_agency'] );
				$saved = es_save_agency( $data, $post_id );

				if ( ! is_wp_error( $saved ) ) {
					if ( $is_new ) {
						$redirect = add_query_arg( array( 'screen' => 'edit-agency', 'agency_id' => $post_id ) );
					}
				} else {
					es_set_flash( 'prop-management', $saved->get_error_message(), 'error' );
				}
			} else {
				es_set_flash( 'prop-management', __( 'Your subscription plan doesn\'t allow to publish more agencies.', 'es' ), 'error' );
			}
		}
...
/**
 * Save agency via ajax action.
 *
 * @return void
 */
function es_pm_ajax_save_agency_handler() {
	$error = __( 'Invalid security nonce. Please, reload the page and try again.', 'es' );
	$error_template = '<span class="es-icon es-icon_close"></span><h4>%s</h4><p>%s</p>';
	$error_title = _x( 'Error!', 'popup error title', 'es' );
	$message = sprintf( $error_template, $error_title,  $error );
	...
	if ( check_ajax_referer( 'es_save_agency_ajax', 'es_save_agency_ajax' ) ) {
		$post_id = intval( filter_input( INPUT_POST, 'post_id' ) );
		$is_new = empty( $post_id );
		$data = es_clean( $_POST['es_agency'] );
		$success_template = '<span class="es-icon es-icon_check-mark"></span><h4>%s</h4>';
		...
		if ( ! $is_new || ( es_user_can_publish_agencies( get_current_user_id() ) || $is_new ) ) {
			...
			if ( ! empty ($_FILES['avatar']) ) {
				...
				if (! $is_new ) {
					...
					set_post_thumbnail( $saved, $attachment_id );	
				}
			}
			...
			if ( is_wp_error( $saved ) ) {
				...
				if ( $is_new ) {
					$message = __( 'New agency was submitted successfully.', 'es' );
				} else {
					$message = __( 'The agency was updated successfully.', 'es' );
				}
				$response = es_success_ajax_response( sprintf( $success_template, $message ) );
				$response['post_id'] = $saved;

				$profile_page = get_post_status( ests( 'profile_page_id' ) ) == 'publish' ? ests( 'profile_page_id' ) : null;
				$profile_page = get_post_status( ests( 'agency_management_page_id' ) ) == 'publish' ? ests( 'agency_management_page_id' ) : $profile_page;

				if ( $profile_page ) {
					$response['redirect_url'] = add_query_arg( array( 'tab' => 'my-agencies' ), get_permalink( $profile_page ) );
					$response['redirect_timeout'] = 3000;
				}
			}
		} else {
			$error_template = '<span class="es-icon es-icon_error"></span><h4>%s</h4>';
			$message = __( 'Your subscription plan doesn\'t allow to publish more agencies.', 'es' );
			$response = es_error_ajax_response( sprintf( $error_template, $message ) );
		}
	...
	$response = apply_filters( 'es_pm_ajax_save_agency_handler_response', $response );

	wp_die( json_encode( $response ) );
}

add_action( 'init', 'es_pm_save_agency_handler' );
add_action( 'wp_ajax_es_ajax_save_agency', 'es_pm_ajax_save_agency_handler' );
...
function es_attach_property_agent( $post_id ) {
	if ( es_is_property( $post_id ) && current_user_can( 'agent' ) ) {
		...
		if ( empty( $agents ) || ( $agent_user->post_id && is_array( $agents ) && ! in_array( $agent_user->post_id, $agents ) ) ) {
            ...
            $agent_post = es_get_agent( $agent_user->post_id );
            ...
            if ( ( is_array( $assign_type ) && ! in_array( 'agency', $assign_type ) ) || ! $assign_type ) {
                if ( $agent_post instanceof Es_Agent_Post && $agent_post->has_agency() ) {
	                $assign_type[] = 'agency';
                    $agency_id = $agent_post->agency_id;
                }
            }
            ...
            if ( in_array( 'agency', $assign_type ) && ! empty( $agency_id ) ) {
	            $property->save_field_value( 'agency_id', $agency_id );
            }
		}
	}
}
 ...
 * @return mixed
 */
function es_property_default_fields_additional_currencies( $fields ) {
	if ( ests( 'additional_currencies' ) && ests( 'is_additional_currencies_enabled' ) ) {
		foreach ( ests( 'additional_currencies' ) as $additional_currency ) {
            $key = sprintf( 'price_%s', es_strtolower( $additional_currency['currency'] ) );
			$fields[ $key ] = array(
				'fb_settings' => array(
				...
				'search_support' => true,
				'formatter' => 'price',
				'label' => sprintf( __( 'Price %s', 'es' ), $additional_currency['currency'] ),
				'is_visible' => 0,
				'tab_machine_name' => 'basic-facts',
				'frontend_tab_machine_name' => 'description',
				'attributes' => array(
					'placeholder' => $additional_currency[ 'currency_sign' ] ?
                        $additional_currency[ 'currency_sign' ] : $additional_currency[ 'currency' ],
				),
				'order' => is_admin() ? 1 : 20,
			);
		}
...
add_filter( 'es_property_default_fields', 'es_property_default_fields_additional_currencies' );
 ...
 * @return mixed
 */
function es_alter_the_price_field( $value, $field, $post ) {
    if ( 'price' == $field ) {
        $currency = es_get_site_currency();

        if ( $currency != ests( 'currency' ) && ests( 'is_additional_currencies_enabled' ) ) {
	        $property = es_get_entity_by_id( $post );
	        $key = sprintf( 'price_%s', es_strtolower( $currency ) );
	        $value = $property->{$key};
        }
    }
...
add_filter( 'es_get_the_field', 'es_alter_the_price_field', 10, 3 );
