...
function es_agent_field_render( $field_key, $field_config = array() ) {
	...
	if ( $field_info = $entity::get_field_info( $field_key ) ) {
		...
		if ( ! is_admin() ) {
            ...
            if ( $field_key == 'user_id' && ! empty( $field_config['value'] ) ) {
                if ( $user = es_get_user_entity( $field_config['value'] ) ) {
	                $field_config['after'] = sprintf( "<a href='%s' target='_blank'>%s%s</a></div>",
		                get_edit_user_link( $field_config['value'] ),
                        "<span class='es-icon es-icon_pencil'></span> ",
                        __( 'Edit user', 'es' ) );

	                $field_config['options'][ $user->get_id() ] = $user->get_full_name();
                }
            }

            if ( $field_key == 'has_user' && ! empty( $entity->user_id ) ) {
                unset( $field_config['options'][0] );
            }
        }
...
function es_agency_field_render( $field_key, $field_config = array() ) {
	...
	if ( ! es_is_agency( $entity->get_id() ) ) {
		$entity = es_get_agency( - 1 );
	}

	if ( $field_info = $entity::get_field_info( $field_key ) ) {
		if ( ! is_admin() && ! empty( $field_config['system'] ) ) {
			$name = "es_agency[system][{$field_key}]";
		} else {
			$name = "es_agency[{$field_key}]";
		}

		if ( ! is_admin() && ! empty ( $field_config['type'] ) ) {
			if ( $field_config['type'] == 'media' ) {
				unset( $field_config['before'] );
				$field_config['type'] = 'avatar';
			}
		}
 ...
 * @param array $field_config
 */
function es_order_field_render( $field_key, $field_config = array() ) {
	$entity = es_get_order( get_the_ID() );

	if ( ! es_get_order( $entity->get_id() ) ) {
		$entity = es_get_order( - 1 );
	}

	if ( $field_info = $entity::get_field_info( $field_key ) ) {

		$name = "es_order[{$field_key}]";
...
function es_save_order( $data, $post_id = null ) {
	...
	if ( $system ) {
		...
		if ( $post_id && ! is_wp_error( $post_id ) ) {
			...
			do_action( 'es_after_save_order', $post_id );
		}
	}
...
function es_save_agency( $data, $post_id = null ) {
   $data = es_parse_args( $data, array(
	   'system' => array(
		   'post_type' => 'agency',
		   'post_status' => 'publish',
   ...
   if ( ! empty( $post_id ) ) {
	   ...
	   if ( ! current_user_can( 'create_agencies' ) ) {
		   $system = false;
		   $post_id = new WP_Error( -1, __( 'You are not allowed to do this action', 'es' ) );
	   }
   }
 ...
 * @return string
 */
function es_translation_fb_option( $field, $key ) {
	if ( defined( 'ICL_SITEPRESS_VERSION' ) && class_exists('SitePress' ) ) {
		$field_info = es_property_get_field_info( $key );
		$data_values = unserialize( $field_info[ 'values' ] );
		if ( $data_values  !== false) {
			foreach ( $data_values as $item ) {
				if ( __(  $item['value'], 'es' ) == $field ) {
					$field = $item[ 'value' ];
				}
			}
		}
 ...
 * @return mixed
 */
function es_get_site_currency() {
	return ! empty ( $_COOKIE['currency'] ) ? $_COOKIE['currency'] : ests( 'currency' );
}
