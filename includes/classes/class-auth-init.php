...
class Es_Auth {
		...
		if ( 'approve-user' == $action && ! empty( $email ) && ! empty( $key ) ) {
			...
			if ( $user instanceof WP_User && ( $user_instance = es_get_user_entity( $user->ID ) ) ) {
				if ( $user_instance->is_active() ) {
					...
					if ( user_can( $user->ID, 'agent' ) && empty ( ests ( 'is_automatic_approval_of_agents_publication' ) ) ) {
						es_set_flash( 'authenticate', __( 'User successfully approved, wait for confirmation from the administrator.', 'es' ) );
						$user_instance->activate();
						$user_instance->change_status( Es_Agent_User::STATUS_PENDING_APPROVAL );
					} else {
						$user_instance->activate();
	...
	public static function validate_login( $user, $username ) {
		if ( $user instanceof WP_User && $user->has_cap( 'agent' ) ) {
			...
			if ( ! $user_instance->is_active() ) {
				...
				if ( $user_instance->is_pending_approval() ) {
					$user = new WP_Error( 'disabled_user', __( 'The user is awaiting administrator approval.' ) );
				}
			}
		}
	...
	public static function create_agent_post( $user_id, $auth_instance ) {
		...
		if ( stristr( $auth_network, 'agent' ) ) {
			...
			if ( ! is_wp_error( $agent_post_id ) && $agent_post_id ) {
				...
				if ( ests ( 'is_automatic_approval_of_agents_publication' ) ) {
					$agent_user->activate();
				} else {
					$agent_user->change_status( Es_Agent_User::STATUS_PENDING_APPROVAL );
				}
				
				update_user_meta( $user_id, 'es_agent_user_post_id', $agent_post_id );
	...
	public static function register_agent( $user_data ) {
		$user_data['has_user'] = ! empty( $user_data['user_id'] );
		...
		if ( $agent_post_id && ! is_wp_error( $agent_post_id ) ) {
			...
			if ( $user instanceof Es_User && $user->get_id() ) {
				...
				$entity->save_field_value( 'has_user', 1 );
			} else {
