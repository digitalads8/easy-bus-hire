 ...
 */
class Es_Agency_Management_Shortcode extends Es_Shortcode {
	...
	public static function get_tabs() {
		...
		if ( $sections = $sections_builder::get_items( 'agency' ) ) {
			$i = 1;
			foreach ( $sections as $section_id => $section ) {
				if ( es_can_render_tab( 'agency', $section_id ) ) {
					$tabs[ $section_id ] = array(
						'label'    => "<span class='es-tabs__numeric'>{$i}</span>" . $section['label'],
						'template' => es_locate_template( sprintf( 'front/shortcodes/agency-management/tabs/%s.php', $section_id ) ),
						'action'   => 'es_agency_pm_tab',
					);
					$i++;
				}
			}
	...
	public static function tab_content( $item, $section_id ) {
		...
		if ( $fields = $fields_builder::get_frontend_tab_fields( $section_id, 'agency' ) ) {
			...
			foreach ( $fields as $field_key => $field_config ) {
				...
				$field_config = apply_filters( 'es_pm_before_render_agency_field', $field_config, $field_key, $section_id );

				es_agency_field_render( $field_key, $field_config );
			}
	...
	public function get_content() {
		...
		if ( ! has_action( 'es_agency_pm_tab', array( __CLASS__, 'tab_content' ) ) ) {
			add_action( 'es_agency_pm_tab', array( __CLASS__, 'tab_content' ), 10, 2 );
		}
		...
		if ( ! es_user_can_publish_agencies() ) {
			...
			es_set_flash( 'prop-management', __( 'You’re hit your agencies limits.' ) . sprintf( wp_kses( __( 'Please <a href="%s">upgrade your plan</a> to publish more agencies.', 'es' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( es_get_page_url( 'pricing' ) ) ), 'warning' );
		}

		wp_enqueue_script( 'es-property-metabox' );

		if ( ! $screen || $screen == 'agency' ) {
			es_load_template( 'front/shortcodes/agency-management/listings.php', array(
				'config' => $this->get_attributes()
			) );
		} else if ( $screen == 'add-new-agency' ) {
			if ( ! ests( 'is_frontend_management_enabled' ) ) {
				...
				if ( es_current_user_can_or_die( 'create_es_properties' ) ) {
					if ( es_user_can_publish_agencies() ) {
						...
						es_load_template( 'front/shortcodes/agency-management/agency-form.php', array(
		...
		} else if ( $screen == 'edit-agency' ) {
			$agency_id = intval( filter_input( INPUT_GET, 'agency_id' ) );

			if ( es_current_user_can_or_die( 'edit_post', $agency_id ) ) {
				$query = new WP_Query( array( 'post_type' => 'agency', 'p' => $agency_id ) );
				if ( $query->have_posts() ) {
	...
	public static function get_shortcode_name() {
		return array( 'es_agency_management', 'agency_management' );
	}
}
