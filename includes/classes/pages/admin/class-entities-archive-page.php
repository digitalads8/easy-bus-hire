...
abstract class Es_Entities_Archive_Page {
	...
	public static function entities_actions() {
		...
		if ( ! empty( $_GET['_nonce'] ) && wp_verify_nonce( $_GET['_nonce'], $nonce ) ) {
			...
			$redirect_uri = es_get( '_redirect' ) ? es_get( '_redirect' ) : wp_get_raw_referer();
					...
					case 'publish':
						foreach ( $posts_ids as $post_id ) {
							if ( current_user_can( $post_type->cap->publish_posts ) ) {
								...
								if ( ! es_user_can_publish_featured_listings( $post_author_id ) && $property->is_featured() && current_user_can( 'agent' ) ) {
									es_set_flash( 'prop-management', __( 'Sorry, you\'ve reached the limit for featured listings in your subscription plan.' ) . ' ' . sprintf( wp_kses( __( 'Please  <a style="color: #03A9F4" href="%s">Purchase </a> additional listings or Edit existing ones.', 'es' ), array(  'a' => array( 'href' => array(), 'style' => array() ) ) ), esc_url( es_get_page_url( 'pricing' ) ) ), 'error' );
								} else {
