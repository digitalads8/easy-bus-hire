...
class Es_Migrations {
	...
	public static function execute() {
		...
		if ( ! static::is_executed( 'pro_roles' ) ) {
			...
			if ( $agent_role && $admin_role->has_cap( 'edit_private_es_properties' ) && $agent_role->has_cap( 'edit_es_property' ) ) {
		...
		if ( static::is_executed( 'field-builder-setup-db' ) && ! static::is_executed( 'fb_pdf_support' ) ) {
			$wpdb->query( "ALTER TABLE {$wpdb->prefix}estatik_fb_fields ADD is_pdf_visible INT(1) DEFAULT '1'" );
			$wpdb->query( "ALTER TABLE {$wpdb->prefix}estatik_fb_sections ADD is_pdf_visible INT(1) DEFAULT '1'" );

			$q = static::is_column_exists( "{$wpdb->prefix}estatik_fb_fields", 'is_pdf_visible' );
			$q2 = static::is_column_exists( "{$wpdb->prefix}estatik_fb_sections", 'is_pdf_visible' );
