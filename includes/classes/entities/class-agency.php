...
class Es_Agency extends Es_Post implements Es_Emailed_Entity {
	...
	public static function get_default_fields() {
		...
		$fields = array(
			...
			'_thumbnail_id' => array(
				...
				'button_label' => __( 'Upload file', 'es' ),
	...
	public function save_fields( $data ) {
	    if ( ! empty( $data['agent_id'] ) ) {
	        global $wpdb;
	        $wpdb->delete( $wpdb->postmeta, array( 'meta_key' => 'es_agent_agency_id', 'meta_value' => $this->get_id() ) );
			...
			if ( ! empty( $_FILES['avatar'] ) ) {
				...
				if ( $attachment_id ) {
					if ( ! $attachment_id instanceof WP_Error ) {
						...
						// $data['system']['_thumbnail_id'] = $attachment_id;
					}
				}
			}
