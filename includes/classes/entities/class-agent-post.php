...
class Es_Agent_Post extends Es_Post implements Es_Emailed_Entity {
	...
	public static function get_default_fields() {
		$fields = array(
			...
			'first_name' => array(
				'before' => is_admin() ? '<div id="es-user-pwd" class="es-hidden">' : null,
			...
			'user_id' => array(
				'before' => '<div id="es-user-id" class="es-hidden" style="margin-top: -2rem; margin-bottom: 1rem;">',
				'after' => '</div>',
				'label' => __( 'User', 'es' ),
				'type' => 'select',
				'tab_machine_name' => 'basic-facts',
				'order' => 180,
				'attributes'    => array(
					'placeholder' => __( 'No user', 'es' ),
					'data-placeholder' => _x( 'No User', 'agents filter select', 'es' ),
					'class' => 'js-es-select2-search',
					'data-formatNoMatches' => __( 'Agent not found', 'es' ),
					'data-request' => es_esc_json_attr( array( 'response_format' => 'select2', 'action' => 'get_select2_agents_users', '_ajax_nonce' => wp_create_nonce( 'get_select2_agents_users' ) ) ),
				),
			),
			...
			'status' => array(
				...
				'options' => array(
					...
					Es_User::STATUS_PENDING_APPROVAL => __( 'Pending approval', 'es' ),
				),
			),
...
//				'tab_machine_name' => 'description',
		...
		foreach ( $networks as $network => $label ) {
			...
			$fields[ $network ] = array(
				'label' => sprintf( __( '%s profile link', 'es' ), $label ),
				'type' => 'url',
				'order' => $order,
	...
	public function get_user_status() {
		if ( ! $this->has_user_entity() ) {
			...
			if ( $this->status == -1 ) {
				$status = __( 'Approval email sent', 'es' );
			} elseif ( $this->status == -2 ) {
				$status = __( 'Awaiting approval', 'es' );
			} else {
	...
	public function save_fields( $data ) {
		...
		if ( empty( $data['has_user'] ) ) {
			...
			if ( ! is_wp_error( $status ) ) {
				$this->save_field_value( 'user_id', $status );
