...
class Es_User_Subscription {
	...
	public function __construct( $data = array() ) {
		...
		if ( ! empty( $this->user_id ) ) {
			...
			$this->published_agencies_count =
				(int)get_user_meta( $this->user_id, 'es_published_agencies_count', true );	
		}
	}
	...
	public static function build_from_order( Es_Order $order ) {
		return new static( array(
			...
			'agencies_count' => $order->agencies_count,
			'plan' => $order->plan,
		) );
	}
	...
	public function increase_agencies() {
		$inc = $this->published_agencies_count + 1;
		update_user_meta( $this->user_id, 'es_published_agencies_count', $inc );
		$this->published_agencies_count = $inc;
	}

	public function decrease_agencies() {
		$inc = $this->published_agencies_count - 1;
		$inc = $inc < 0 ? 0 : $inc;
		update_user_meta( $this->user_id, 'es_published_agencies_count', $inc );
		$this->published_agencies_count = $inc;
	...
	public static function build_from_plan( Es_Subscription_Plan $plan, $user_id = null ) {
		...
		return new static( array(
			...
			'agencies_count' => (int)$plan->is_agencies_limited ? $plan->agencies_limit : -1,
		) );
	}
	...
	public static function get_default_fields() {
		$fields = array(
			...
			'published_agencies_count' => array(
				'default_value' => 0,
			),

			'agencies_count' => array(
				'default_value' => 0,
			),
