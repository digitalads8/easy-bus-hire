...
function es_paypal_create_plan_request_body( Es_Subscription_Plan $plan, $interval ) {
	...
	if ( ! empty( $price ) && ( $paypal_product = es_paypal_get_base_product() ) ) {
		...
		if ( $plan->is_free_trial_enabled ) {
			$billing_cycles[] = array(
				'frequency' => array(
					'interval_unit' => strtoupper( $plan->trial_period ),
					'interval_count' => 1,
				),
				'tenure_type' => 'TRIAL',
				'sequence' => $sequence++,
 ...
 * @return string
 */
function es_get_add_new_agency_url() {
	$link = false;

	if ( es_user_can_publish_agencies() ) {
		$link = add_query_arg( 'screen', 'add-new-agency' );
	} else {
		if ( ests( 'plans' ) ) {
			$link = es_get_page_url( 'pricing' );
		} else if ( es_is_otp_enabled() ) {
	...
	return apply_filters( 'es_get_add_new_agency_url', $link );
}
...
function es_set_user_subscription( $data ) {
	...
	if ( ! es_has_user_meta( $data['user_id'], 'es_published_listings_count' ) ) {
		...
		update_user_meta( $data['user_id'], 'es_published_agencies_count', 0 );
	}
...
function es_cancel_user_subscription( $user_id = null ) {
	...
	delete_user_meta( $user_id, 'es_published_agencies_count' );
...
function es_get_user_subscription( $user_id = null, $use_default_subscription = true ) {
	...
	if ( ! $subscription && $default_plan ) {
		$subscription = Es_User_Subscription::build_from_plan( $default_plan, $user_id );
	}
 ...
 * @return bool
 */
function es_user_can_publish_agencies( $user_id = null ) {
	$user_id = $user_id ? $user_id : get_current_user_id();
	if ( ! ests( 'is_subscriptions_enabled' ) ||  ! ests( 'is_subscriptions_enabled_agencies' ) || user_can( $user_id, 'manage_options' ) ) return true;

	if ( ! es_user_has_active_subscription( $user_id ) ) return false;
	$subscription = es_get_user_subscription( $user_id );

	if ( $subscription->agencies_count == -1 ) {
		$can_publish = true;
	} else {
		if ( $subscription->published_agencies_count < 0 ) {
			$can_publish = true;
		} else {
			$can_publish = $subscription->published_agencies_count < $subscription->agencies_count;
		}
	}

	return apply_filters( 'es_user_can_publish_agencies', $can_publish, $subscription, $user_id );
}
 ...
 *
 */
function es_update_agent_publish_posts_count($user_login, $user) {

	if ( es_user_has_active_subscription( $user->ID ) ) {
		if ( ests( 'is_subscriptions_enabled' ) ) {
			if (in_array('agent', $user->roles)) {
				...
				$properties_count = count(get_posts($args));

				$term_id = es_get_featured_term_id();
				...
				if ( ests( 'is_subscriptions_enabled_agencies' ) ) {
					$args_agencies = array(
						'author'         => $user_id,
						'post_type'      => 'agency',
						'post_status'    => 'publish',
					...
					$agencies_count = count(get_posts($args_agencies));
					update_user_meta( $user_id, 'es_published_agencies_count', $agencies_count );
				}
				
				$properties_featured_count = count(get_posts($args_featured));
				update_user_meta( $user_id, 'es_published_featured_listings_count', $properties_featured_count );
				update_user_meta( $user_id, 'es_published_listings_count', $properties_count );
			}
		}
	}
}

add_action('wp_login', 'es_update_agent_publish_posts_count', 10, 2);

function es_function_on_page_profile_load() {
	$profile_page_id = ests( 'profile_page_id' );
	$property_management_page_id = ests( 'property_management_page_id' );
	if ( ests( 'is_subscriptions_enabled' ) ) {
		if (is_page($profile_page_id) || is_page($property_management_page_id)) {
			$current_user = wp_get_current_user();
			if (!empty($current_user->user_login)) {
				es_update_agent_publish_posts_count($current_user->user_login, $current_user );
			}
		}
	}
}

add_action('template_redirect', 'es_function_on_page_profile_load');


function add_agent_capabilities_415() {
    $capabilities = array(
        'read_agency',
        'edit_agency',
        'create_agencies',
...
add_action('init', 'add_agent_capabilities_415');
