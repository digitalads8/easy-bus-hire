...
if ( ! function_exists( 'es_get_properties_query_args' ) ) {
    ...
    function es_get_properties_query_args( $args = array() ) {
		    ...
		    switch ( $sort ) {
			    ...
			    case 'lowest_price':
                    ...
                    $meta_query['price_exists'] = array(
                        'relation' => 'OR',
                        array(
                            'key' => ests( 'is_additional_currencies_enabled' ) && $_COOKIE[ 'currency' ] != ests( 'currency' )  ? 'es_property_price_' . es_strtolower( $_COOKIE['currency'] ) : 'es_property_price',
                            'compare' => 'EXISTS',
                        ),
                        array(
                            'key' => ests( 'is_additional_currencies_enabled' ) && $_COOKIE[ 'currency' ] != ests( 'currency' )  ? 'es_property_price_' . es_strtolower( $_COOKIE['currency'] ) : 'es_property_price',
        ...
        foreach ( $range_fields as $range_field ) {
            ...
            if ( ! empty( $_COOKIE['currency'] ) && $range_field == 'price' && ests( 'currency' ) != $_COOKIE['currency'] && ests( 'is_additional_currencies_enabled' ) ) {
                $range_field_key = 'price_' . es_strtolower( $_COOKIE['currency'] );
            }  else {
                $range_field_key = $range_field;
            }

	        if ( isset( $args['fields'][ 'from_' . $range_field ] ) && strlen( $args['fields'][ 'from_' . $range_field ] ) ) {
		        $meta_query[ $range_field ] = array(
			        'key' => $property->get_entity_prefix() . $range_field_key,
        ...
        $search_by = ! empty( $args['fields']['search_by'] ) ? $args['fields']['search_by'] : null;
       
        if ( isset( $args['fields']['search_context'] ) ) {
	        unset( $args['fields']['search_context'] );
        }
        
        if ( isset( $args['fields']['search_by'] ) ) {
	        unset( $args['fields']['search_by'] );
        }
        if ( ! empty( $args['fields'] ) ) {
            foreach ( $args['fields'] as $field => $value ) {
                ...
                if ( $field == 'address' && ( $search_context == 'pm' || $search_by == 'keywords' ) ) {
                    $field = 'keywords';
                    $value = array( $value );
                }
                                ...
                                if ( 'address' == $field ) {
                                    ...
                                    if ( is_array( $value ) ) {
	                                    foreach ( $value as $field_value ) {
                                            $meta_query[ $field ][] = array( 'key' => $property->get_entity_prefix() . $field, 'value' => es_translation_fb_option( $field_value, $field ) );
	                                    }
...
if ( ! function_exists( 'es_get_additional_currencies' ) ) {

    /**
     * Display additional currencies on listings page.
     *
     * @param $sort
     * @return void
     */
    function es_get_additional_currencies() {
        if ( $additional_currencies = ests( 'additional_currencies' ) ) {
	        $signs = ests_values( 'currency_sign' );

	        array_unshift( $additional_currencies, array(
                'currency' => ests( 'currency' ),
                'currency_sign' => ests( 'currency_sign' ),
            ) );

	        foreach ( $additional_currencies as $values ) {
		        $currency_array[ $values['currency_sign'] ] = sprintf( '%s %s', $signs[ $values['currency'] ], $values['currency'] );
	        }

	        if ( ! empty( $currency_array ) ) : ?>
                <div class="es-additional_currencies">
			        <?php es_framework_field_render( 'currencies', array(
				        'type' => 'select',
				        'value' =>  es_get_site_currency(),
				        'options' =>  apply_filters( 'es_get_dropdown_currencies', $currency_array ),
				        'attributes' => array(
					        'class' => 'js-es-currencies'
				        )
			        ) ); ?>
                </div>
	        <?php endif;
...
add_action( 'es_additional_currencies', 'es_get_additional_currencies' );
