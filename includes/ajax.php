...
function es_ajax_management_delete_property_popup() {
	if ( check_ajax_referer( 'es_management_delete_property_popup', '_nonce' ) ) {
		$property_id = intval( filter_input( INPUT_GET, 'entity_id' ) );
...
function es_ajax_management_delete_agency_popup() {
	if ( check_ajax_referer( 'es_management_delete_property_popup', '_nonce' ) ) {
		$agency_id = intval( filter_input( INPUT_GET, 'entity_id' ) );

		if ( current_user_can( 'delete_es_agency', $agency_id ) ) {
			$query = new WP_Query( array(
				'post_type' => 'agency',
				'p' => $agency_id,
			...
			if ( $query->have_posts() ) {
				$title = __( "Remove this agency?", 'es' );
				...
				$content = "<div class='es-magnific-popup es-magnific-popup--delete-action'>
					...
					<div class='es-magnific-listing'>
						<div class='es-magnific-buttons'>
							<a href='#' data-trigger-click='.mfp-close' class='es-btn es-btn--default'>" . __( 'Cancel', 'es' ) . "</a>
							<a href='" . es_get_action_post_link( $agency_id, 'delete' ) . "' class='es-btn es-btn--secondary'><span class='es-icon es-icon_trash'></span>" . __( 'Delete agency', 'es' ) . "</a>
						</div>
					</div>
				</div>";

				$response = apply_filters( 'es_ajax_management_delete_agency_response', array(
					'status' => 'success',
					'message' => $content,
				) );
...
add_action( 'wp_ajax_es_management_delete_agency_popup', 'es_ajax_management_delete_agency_popup' );
...
/**
 * Return agent users for select2 field.
 *
 * @return void
 */
function es_ajax_get_select2_agents_users() {
	if ( check_ajax_referer( 'get_select2_agents_users' ) && current_user_can( 'manage_options' ) ) {
		$args = array(
			'role' => 'agent',
			'number' => 10,
		);
		...
		if ( ! empty( $users ) ) {
			foreach ( $users as $user ) {
				$user = es_get_user_entity( $user->ID );
				$result[] = array( 'id' => $user->get_id(), 'text' => $user->get_full_name() );
			}
		}
...
add_action( 'wp_ajax_get_select2_agents_users', 'es_ajax_get_select2_agents_users' );

/**
 * Set site currency via ajax action.
 *
 * @return void
 */
function es_ajax_set_currency() {
	if ( check_ajax_referer( 'es_set_currency' ) ) {
		if ( $currency = es_post( 'currency' ) ) {
			es_setcookie( 'currency', $currency );
		}
	}

	wp_die();
}
add_action( 'wp_ajax_es_set_currency', 'es_ajax_set_currency' );
add_action( 'wp_ajax_nopriv_es_set_currency', 'es_ajax_set_currency' );
