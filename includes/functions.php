		...
		switch ( $format ) {
            ...
            case 'price-area':
                if ( $value ) {
					...
					if ( ! empty( $_COOKIE['currency'] ) && ests( 'is_additional_currencies_enabled' ) && $args['is_single_meta'] == 1 ) {
						$additional_currencies = ests( 'additional_currencies' );
						foreach ( $additional_currencies as $additional_info ) {
							if ( $additional_info['currency'] == $_COOKIE['currency'] ) {
								$dec = es_clean_string( $additional_info[ 'currency_dec' ] );
								$dec_num = es_clean_string( $additional_info[ 'currency_dec_num' ] );
								$sup = es_clean_string( $additional_info[ 'currency_sup' ] );
								$price_format = $sup . $dec;
								$sign = $signs[ $additional_info[ 'currency_sign' ] ];
								$position = $additional_info['currency_position'];
								$currency = $sign ? $sign : $additional_info['currency'];
							}
						}
					}
...
if ( ! function_exists( 'es_get_entities_table_instance' ) ) {
	...
	function es_get_entities_table_instance( $entity_type, $args = array() ) {
		...
		if ( $entity_type == 'agency' ) {
			$instance = new Es_Agencies_Table( $args );
		}
...
function es_get_profile_tabs() {
	$tabs = array(
		...
		'my-agencies' => array(
			'template' => es_locate_template( 'front/shortcodes/profile/tabs/my-agencies.php' ),
			'label' => __( 'My Agencies', 'es' ),
			'icon' => "<span class='es-icon es-icon_agency'></span>",
			'id' => 'my-agencies',
		),
		'requests' => array(
